...
void main() {
  ...
  group('Multi-Session: Message Isolation', () {
    ...
    test(
      'clearConversation only deletes target conversation messages',
      () async {
        await convDao.upsertConversation(
          ConversationsCompanion(
            conversationId: const Value('conv_a'),
  ...
  group('Multi-Session: ensureConversation Idempotency', () {
    ...
    test(
      'ensureConversation always generates UUID, never uses accountId',
      () async {
        await convRepo.ensureConversation(
          accountId: 'openclaw',
          type: 'ai',
  ...
  group('Multi-Session: createConversation UUID', () {
    ...
    test('createConversation sets lastMessageAt for list ordering', () async {
      final before = DateTime.now().millisecondsSinceEpoch;
      final id = await convRepo.createConversation(
        accountId: 'openclaw',
        type: 'ai',
        name: 'Fresh Chat',
      );
      ...
      expect(conv!.lastMessageAt, greaterThanOrEqualTo(before));
      expect(conv.lastMessageAt, lessThanOrEqualTo(after));
    });
  });
  ...
  group('Multi-Session: getConversationsByAccount', () {
    test('returns all conversations under the same accountId', () async {
      ...
      final openclawConvs = await convDao.getConversationsByAccount('openclaw');
      final nanobotConvs = await convDao.getConversationsByAccount('nanobot');

      expect(openclawConvs.length, 2);
      expect(nanobotConvs.length, 1);
      expect(openclawConvs.every((c) => c.accountId == 'openclaw'), isTrue);
  ...
  group('Multi-Session: Delete Conversation Workflow', () {
    test(
      'clearing messages + deleting conversation leaves no orphans',
      () async {
        await convDao.upsertConversation(
          ConversationsCompanion(
            conversationId: const Value('conv_a'),
  ...
  group('Multi-Session: Backward Compatibility', () {
    test(
      'conversation with conversationId == accountId works (legacy mode)',
      () async {
