...
**[功能优化]** 优化欢迎页与登录入口。
- 优化首次启动欢迎页，桌面端和移动端都会让主要登录入口居中展示，手动配置服务器保持为次要入口。
- 精简登录按钮文案，并保持语言切换和协议页脚在小屏幕上可响应滚动。

**[问题修复]** 同版本更新提示处理。
- 当服务端返回的 `update_available` 与当前客户端语义版本一致时忽略该提示，避免 `1.1.33+84` 误弹 `v1.1.33` 升级弹窗。

## v1.1.33 (2026-05-21)

**[新功能]** 发布兼容性与更新信息可见性。
- 新增 server-client 版本兼容性检查，客户端可根据当前 Server 版本展示更新指引。
- 更新流程中新增明确的版本信息展示，方便确认当前发布状态。

**[功能优化]** README 产品展示。
- 更新 README 产品截图，展示当前桌面端和移动端界面。
- 刷新交流群二维码，并精简公开功能说明。

**[问题修复]** macOS 内部 DMG 打包与 SkillHub 详情提示。
- 修复内部 macOS DMG 创建流程，在压缩前清理 filesystem event 元数据。
- SkillHub 状态提示支持点击触发，触屏设备上也能查看内置技能路径。
